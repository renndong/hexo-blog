...
Note:64位系统中，不再使用`int 0x80`来进行系统调用，取而代之的是`syscall`指令
...
14| sys_rt_sigprocmask| int how| sigset_t *nset| sigset_t *oset| size_tsigsetsize| |  
...
24| sys_sched_yield| | | | | |  
25| sys_mremap| unsigned long addr| unsigned long old_len| unsigned longnew_len| unsigned long flags| unsigned long new_addr|  
...
44| sys_sendto| int fd| void *buff| size_t len| unsigned flags| structsockaddr *addr| int addr_len  
